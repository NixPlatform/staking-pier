...
contract TokenGeyser is IStaking, Ownable {
    ...
    function _unstake(uint256 amount) private returns (uint256) {
        ...
        uint256 amountMinusLock = amount.mul(99).div(100);
        uint256 amountLocked = amount.sub(amountMinusLock);
        // interactions
        require(_stakingPool.transfer(msg.sender, amountMinusLock),
            'TokenGeyser: transfer out of staking pool failed');
        require(_stakingPool.transfer(address(this), amountLocked),
            'TokenGeyser: transfer out of staking pool failed');
        require(_unlockedPool.transfer(msg.sender, rewardAmount),
            'TokenGeyser: transfer out of unlocked pool failed');

        emit Unstaked(msg.sender, amountMinusLock, totalStakedFor(msg.sender), "");
